...
export function ArticleHeader({
  ...
  authorUrl,
  authorBio,
  avatarSrc = '/raul-alvarez-sm.webp',
  lang,
  editorId,
}: ArticleHeaderProps) {
  const resolvedAuthorUrl = authorUrl ?? (lang === 'es' ? '/sobre-mi' : '/about')
  return (
    <header className="mb-10" {...(editorId ? { id: editorId } : {})}>
...
const FOOTER_I18N = {
  es: {
    role: 'AI Architect · AI Solutions Engineer · Head of AI @ Aircury',
    bio: 'Perfil, proyectos y casos técnicos en raul-alvarez.es — Granada, remoto EU.',
    copyright: 'Todos los derechos reservados.',
  },
  en: {
    role: 'AI Architect · AI Solutions Engineer · Head of AI @ Aircury',
    bio: 'Profile, projects, and technical case studies on raul-alvarez.es — Granada, EU remote.',
    copyright: 'All rights reserved.',
  },
} as const

export function ArticleFooter({ lang, editorId }: ArticleFooterProps) {
  const f = FOOTER_I18N[lang]
  return (
    <footer className="mt-16 pt-8 border-t border-border" {...(editorId ? { id: editorId } : {})}>
      <div className="flex items-start gap-3 mb-6">
        ...
        />
        <div>
          <p className="font-medium text-foreground">{DISPLAY_NAME}</p>
      ...
      <div className="flex gap-3 mb-8">
        <a href={LINKEDIN_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0A66C2]/10 border border-[#0A66C2]/20 text-sm font-medium text-[#0A66C2] hover:bg-[#0A66C2]/20 transition-colors">
        ...
        <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm font-medium text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors">
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>&copy; {new Date().getFullYear()} {DISPLAY_NAME}. {f.copyright}</span>
