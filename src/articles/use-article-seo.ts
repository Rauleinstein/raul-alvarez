...
export function useArticleSeo(opts: ArticleSeoOpts) {
  useEffect(() => {
    ...
    upsertMeta('name', 'author', 'Raúl Alvarez')
    ...
    upsertMeta('property', 'og:site_name', 'raul-alvarez.es')
    ...
    upsertMeta('property', 'article:author', LINKEDIN_URL)
    ...
    for (const { hreflang, href } of [
      ...
      { hreflang: 'x-default', href: `https://raul-alvarez.es/${defaultSlug}` },
    ]) {
...
export function useHomeSeo({ lang, title, description }: { lang: string; title: string; description: string }) {
  useEffect(() => {
    ...
    const canonical = lang === 'en' ? 'https://raul-alvarez.es/en' : 'https://raul-alvarez.es/'
